  ...
  try {
    ...
    for (let attempt = 0; attempt < 2; attempt++) {
      ...
      try {
        ...
        return NextResponse.json({ error: `Binance futures returned non-JSON: ${txtAttempt.slice(0, 200)}` }, { status: 200 });
      }
    ...
    if (!res) {
      return NextResponse.json({ error: "Binance futures query failed before request dispatch." }, { status: 200 });
    }
